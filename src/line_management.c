...
/*   Updated: 2017/11/21 22:28:32 by ggane            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		draw_one_almost_vertical_line(t_data *d, t_dot *p1, t_dot *p2)
{
	...
	yn = get_min_coordinate(p1->y, p2->y);
	max = get_max_coordinate(p1->y, p2->y);
	while (yn <= max)
	{
...
static void		draw_one_almost_horizontal_line(t_data *d, t_dot *p1, t_dot *p2)
{
	...
	xn = get_min_coordinate(p1->x, p2->x);
	max = get_max_coordinate(p1->x, p2->x);
	while (xn <= max)
	{
